	...
	{   
		...
		if(!empty($result))
		{
			...
			if(empty($price_list_obj->recurring))
			{
				...
				// XERO INVOICE
				$new_trans_id = $result;
				$patient_info = $this->account->get_record($input_acc_id,false);
				...
				$request = array(
							  array(
								  "Type"=>"ACCREC",
								  "Reference"=>$new_trans_id . ' - Balance',
								  "CurrencyCode"=>$price_list_obj->currency,
								  "Contact" => array(
								  ...
								  "LineItems"=> array(
									  "LineItem" => array(
														array(
															...
															"Quantity" => number_format($input_qty,2),
															"UnitAmount" => number_format($price_list_obj->unit_price ,2),
					...
					$inv_result = $xero->Invoices( $request );
					...
					// XERO INVOICE PDF
					if(!$this->invoice_config['send_email'] || !is_dir($this->invoice_config['path'])) return;
					...
					if (!valid_email($recipient_email)) return;
					
					$xero = new Xero(XERO_KEY, XERO_SECRET,PUBLIC_CERT_PATH,PRIVATE_KEY_PATH,'json');
					$pdf_invoice = $xero->Invoices($new_invoice_id, '', '', '', 'pdf');	
			
					$path = $this->invoice_config['path'] . $new_invoice_number . '.pdf';
					$fh  = fopen($path, 'w+');
					...
					$this->email->subject($new_invoice_number);
